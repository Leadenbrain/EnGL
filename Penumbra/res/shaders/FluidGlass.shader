...
vec3 envMap(vec3 rd, vec3 n){
    ...
    vec3 col = vec3(0.1); //tpl(iChannel1, rd*4., n);
...
void main(){
	...
	// Camera Setup.
    float speed = 4.;
    // vec3 o = vec3(-u_X,0.0,u_Z);
    vec3 o = camPath(u_Time*speed); // Camera position, doubling as the ray origin.
    //vec3 lk = o + vec3(0.5-u_Move.x, u_Move.y, 1.0);
    vec3 lk = camPath(u_Time*speed+ 0.25);  // "Look At" position.
    vec3 l = camPath(u_Time*speed + 2.) + vec3(0, 1, 0); // Light position, somewhere near the moving camera.
    ...
    if(t<FAR){
        ...
        */
        ...
        float hi = abs(mod(t/1. + u_Time/3., 8.) - 8./2.)*2.;
        col += vec3(.01, .05, 1)*col*1./(.001 + hi*hi*.2);
 		// Similar effect.
        //col += vec3(.01, .05, 1)*col*abs(tan(t/1.5 + u_Time/3.));
