...
vec2 fragCoord = v_Position.xy;

uniform float u_Time;

vec2 iResolution = vec2(1920.0/1080.0, 1.0);


/*

https://www.shadertoy.com/view/WdjSDh

Based on https://www.shadertoy.com/view/llSyDh

Just adapted it to move with music.
...
float hex(in vec2 p){
    ...
    p = abs(p)/abs(sin(2.00*u_Time)+0.5);//*texture(iChannel0, p*.3).x/2.0;
...
float dots(in vec2 p){
    ...
    return length(p)*abs(sin(1.05*u_Time)+0.2);//*texture(iChannel0, p*.03).x*1.05; // Circles.
...
void main(){
    vec2 fragCoord = v_Position.xy/2.0 + 0.5;
    ...
    float res = clamp(iResolution.y, 1920.0/1080.0, 1.); 
    
    // Aspect correct screen coordinates.
	vec2 u = (fragCoord - iResolution.xy*.5);///res/(2.+hex(s.yx/u_Time/1.)*1000.);
    
    // Scaling and moving the screen coordinates.
    vec2 sc = u*4. + s.yx*u_Time/4.0 + s.yx* hex(s.yx*u_Time/8.)*hex(s.yx/u_Time/2.)/10.0;
    ...
    vec4 h = getHex(sc); // + s.yx*u_Time/2.
    ...
    //float aRnd = sin(rnd*6.283 + u_Time*1.5)*.5 + .5; // Animating the random number.
    ...
    float arcBord = hex(q)*abs(sin(1.05*u_Time)+0.2);//* texture(iChannel0, q*.03).x*1.05;
    ...
    vec3 dotCol = bg.zyx*vec3(.4 , .1, .9)/hex(q) / bg.xyz*vec3(.1 , .9, .9)/hex(10.0/q) ;
    vec3 bCol = mix(bg.zyx, bg.yyy, .25);
    bg = mix(bg.yyy, bg.zyx,  0.9)/hex(q);
